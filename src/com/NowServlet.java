...
@WebServlet(
		...
		initParams = { 
				...
				@WebInitParam(name = "jdbcDriver", value = "orcle.jdbc.OracleDriver", description = "orcle드라이버 제공")
		})
public class NowServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		// 초기화 함수(자동 호출 함수)
		// 호출시점 : NowServlet클래스 파일에 대한 최초 요청시 한번만 실행
		// 재실행 : 개발자가 코드를 수정한 경우, 서버에 재시작
		
		// it.co.kr 서버 오픈 > WAS(서블릿 : NowServlet.java)
		// 홍길동 첫 접속자 -> it.co.kr/NowServlet 서버 요청
		// NowServlet 컴파일 > class(실행) > 생성자 호출 -> init 자동호출 -> doGET or doPOST 자동호출
		
		// 김유신 접속자 -> it.co.kr/NowServlet 서버 요청
		...
		System.out.println("최초 요청시 한번만 실행" + drivername);
	}
